...
class DeviceModel:
    ...
    def update_device_model(self):
        try:
            sql = """ update device_model set 
                    model_name = %s,
                    model_number = %s,
                    water_capacity_liters = %s,
                    coffee_capacity_kgs = %s,
                    milk_capacity_kgs = %s,
                    sugar_capacity_kgs = %s,
                    sweetener_capacity_count = %s,
                    cups_capacity_count = %s 
                    where device_model_id = %s """
            val = (self.model_name, self.model_number, self.water_capacity_liters, self.coffee_capacity_kgs,
                   self.milk_capacity_kgs, self.sugar_capacity_kgs, self.sweetener_capacity_count, self.cups_capacity_count, self.device_model_id)
            mycursor.execute(sql, val)
            print("Device has been updated successfully")
